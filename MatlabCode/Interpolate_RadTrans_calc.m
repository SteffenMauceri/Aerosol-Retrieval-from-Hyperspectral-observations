...
%
% note: 
% -radiance is convolved to AVIRIS-NG spectral resolution of approx 5.5 nm
%
% Steffen Mauceri, Jan 2018

clear
 
aerosol = {'2_bahrain_sand_', '2_stratsulf_indoex_', '2_alexander_tropcarbon_'};

rng(12345)%use a seed to reproduce
randNo = rand(100000,2);

for m=aerosol
    load(strcat('data/',string(m),'.mat'))
    ...
    save(strcat('Preprocessing/',string(m),'interp.mat'), 'values', 'radiance', 'wl' )
end
%execute Add_Surface to add different surface spectra to the radiative transfer calculations.
Add_Surface %